import React from "react";
import Image from "next/image";

interface ProjectsData {
  baseUrl: string;
  categories: Record<string, string[]>;
}

export default function ProjectsPage({ data }: { data: ProjectsData }) {
  return (
    <div className="p-2 md:p-4 font-mono">
      <h2 className="text-lg md:text-xl font-bold mb-4 md:mb-6">My Projects</h2>

      <div className="space-y-6 md:space-y-8">
        {Object.entries(data.categories).map(([category, repoList]) => (
          <div key={category} className="mb-6 md:mb-8">
            <h3 className="text-md font-bold mb-2 md:mb-3 border-b border-black dark:border-white pb-1">
              {category}
            </h3>

            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 md:gap-4">
              {repoList.map((repo) => (
                <a
                  key={repo}
                  href={`https://github.com/manavvgarg/${repo}`}
                  target="_blank"
                  rel="noopener noreferrer"
                  className="block border border-black dark:border-white hover:opacity-80 transition-opacity"
                >
                  <Image
                    src={`${data.baseUrl}${repo}`}
                    alt={`${repo} repository`}
                    width={400}
                    height={200}
                    className="w-full"
                  />
                </a>
              ))}
            </div>
          </div>
        ))}
      </div>

      <p className="text-xs md:text-sm mt-4 md:mt-6">
        You can find more projects on my{" "}
        <a
          href="https://github.com/manavvgarg"
          target="_blank"
          rel="noopener noreferrer"
          className="underline"
        >
          GitHub profile
        </a>
        .
      </p>
    </div>
  );
}
